...
function chart2() {
  const url = 'https://data.cityofchicago.org/resource/xq83-jr8c.json';
  
  // Load the data from the URL using d3.json
  d3.json(url).then(function(data) {
    
    // Extract the GHG emissions and energy rating data
    const ghgEmissions = data.map(d => d.total_ghg_emissions_metric_tons_co2e);
    const energyRating = data.map(d => d.chicago_energy_rating);
    ...
    const layout = {
      title: 'GHG Emissions vs. Chicago Energy Rating',
      xaxis: {
        title: 'Chicago Energy Rating'
      },
      yaxis: {
...
function updateCharts(rowId) {
  const selectedData = data.filter(d => d.row_id === rowId)[0];
  chart1(selectedData);
  chart2(selectedData);
  chart3(selectedData);
  chart4(selectedData);
